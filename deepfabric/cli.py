...
@cli.group()
def training():
    """Commands for training metrics auto-logging.

    Training metrics are automatically logged when you install deepfabric.
    Just set DEEPFABRIC_API_KEY and run your training script.
    """


@training.command("status")
def training_status():
    """Show training auto-logging status and configuration."""
    tui = get_tui()

    try:
        tui.console.print("\n[bold]Training Auto-Logging Status[/bold]\n")

        # Auto-logging is always installed via .pth file
        tui.console.print("  Auto-injection: [green]Enabled[/green] (via .pth file)")
        ...
        if os.getenv("DEEPFABRIC_DISABLE_AUTO_LOGGING") == "1":
            tui.console.print(
                "  Status: [yellow]Disabled[/yellow] (DEEPFABRIC_DISABLE_AUTO_LOGGING=1)"
            )
        else:
            tui.console.print("  Status: [green]Active[/green]")

        # Check for API key
        api_key = os.getenv("DEEPFABRIC_API_KEY")
        if api_key:
            tui.console.print("  API Key: [green]Set[/green]")
            tui.console.print("  Metrics will be logged automatically during training.")
        else:
            tui.console.print("  API Key: [yellow]Not set[/yellow]")
            tui.info("\nTo enable logging, set DEEPFABRIC_API_KEY environment variable:")
            tui.console.print("    export DEEPFABRIC_API_KEY=your-api-key")

        # Check API URL
        api_url = os.getenv("DEEPFABRIC_API_URL", "https://api.deepfabric.ai")
        tui.console.print(f"  API URL: {api_url}")

        tui.console.print("\n[bold]To disable auto-logging:[/bold]")
        tui.console.print("    export DEEPFABRIC_DISABLE_AUTO_LOGGING=1")
...
@training.command("disable")
def training_disable():
    """Show how to disable automatic training metrics logging.

    Auto-logging is enabled by default when deepfabric is installed.
    Use environment variables to disable it.
    """
    tui = get_tui()

    tui.console.print("\n[bold]Disabling Training Auto-Logging[/bold]\n")
    tui.console.print("To disable auto-logging, set the environment variable:\n")
    tui.console.print("    export DEEPFABRIC_DISABLE_AUTO_LOGGING=1\n")
    tui.console.print("This will prevent DeepFabric from injecting callbacks into Trainer.")
    tui.console.print("Your training will run normally without any metrics logging.\n")
    tui.console.print("To re-enable, unset the variable or set it to any other value:\n")
    tui.console.print("    unset DEEPFABRIC_DISABLE_AUTO_LOGGING\n")
